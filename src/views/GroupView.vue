<script setup>
  ...
  async function stats() {
    ...
    for (let i in allStats.value) {
      if (i != "" && i != "Incoming" && i != "Rewards Withdrawal" && i != "Staking" && i != "Internal transfer") {
        labels.push(i)
...
<template>
  ...
  <div class="container">
    <div class="container-child">
      ...
      <RouterLink :to="`/${store.group}/${wallet}`" class="wallets" v-for="wallet in treasuryWallets" :key="wallet">
        <div>{{ wallet }}</div>
      </RouterLink>
      <h3 v-if="proposals.length > 0">Proposals</h3>
      <RouterLink :to="`/${store.group}/${wallet}`" class="wallets" v-for="wallet in proposals" :key="wallet">
        <div>{{ wallet }}</div>
      </RouterLink>
    </div>
    <div class="container-child">
