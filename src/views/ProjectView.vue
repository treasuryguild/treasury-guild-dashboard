<script setup>
  ...
  async function getProjectDetails() { // still busy building and testing
    ...
    totalIn.value = 0
    totalOut.value = 0
    header.value = ['Date','TxType','ExchangeRate','Recipients','Link','Fees','Balance']
    ...
    const { lovelace, tokens, tokenAmounts, nfts, nftPicture } = await useGetWallet(walletRender.value)
    lovelaceR.value = parseFloat(lovelace.value).toFixed(2)
    const { tx_id, transaction_id, transaction_date, exchange_rate, tx_json_url, total_tokens, total_amounts, recipients, wallet_balance_after, fee } = await useGetTransactions(project_idRender.value)
    ...
    const { all_contribution_id, all_transaction_id, all_task_creator, all_task_name, all_task_label, all_task_description, all_task_type } = await useGetAllContributions(project_idRender.value)
    ...
    const { all_dist_id, all_contributor_id, all_tokens, all_amounts } = await useGetAllDistributions(project_idRender.value)
    ...
    console.log('test dist tokens',tokens.value,tokenAmounts.value,nfts.value,nftPicture.value)
    ...
    for (let i in txId.value) {
      txrows.value[i] = {Txid:'',Date:'',ExchangeRate:'',Recipients:'',Link:'',TxType:'',Fees:'',Balance:''}
      
      for (let j in txTotalTokens.value[i]) {
        txrows.value[i][(txTotalTokens.value[i][j].toUpperCase())] = txTotalAmounts.value[i][j]
      } 

      if (txType.value[txId.value[i]] == "Incoming" || txType.value[txId.value[i]] == "Rewards Withdrawal") {
      ...
      txrows.value[i].Date = new Date(parseInt(transactionDate.value[i])).toDateString()
      txrows.value[i].ExchangeRate = exchangeRate.value[i]
      txrows.value[i].Recipients = recipientsR.value[i]
      txrows.value[i].TxType = txType.value[txId.value[i]] == "Incoming" || txType.value[txId.value[i]] == "Rewards Withdrawal" ? txType.value[txId.value[i]] : ''
      txrows.value[i].Link = txJsonUrl.value[i]
      txrows.value[i].Fees = parseFloat(feeR.value[i])/1000000
      txrows.value[i].Balance = parseFloat(walletBalanceAfter.value[i]).toFixed(2)
    ...
    console.log('Everything is fine, but is it', txrows.value, txTotalAmounts.value, txTotalTokens.value, headerTokens.value)
    loading.value = false;
  }

  async function getSingleTransaction(txid) { // place txid in await usegetContributions
    const { contribution_id, task_creator, task_name, task_label, task_description, task_type } = await useGetContributions(transactionId.value[0])
    ...
    const { dist_id, contributor_id, tokens, amounts } = await useGetDistributions(contributionId.value[0])
  ...
  function pageBottom() {
    ...
    let scrollInterval = setInterval(function() {
      if (currPos < scrollHeight) {
        count += 1;
        const newPos = Math.round(scrollHeight - (scrollHeight - currPos) * Math.cos(count * scrollStep));
  ...
  const rows = [["Date","TxType","ExchangeRate","Recipients","Link","Fees","Balance", ...headerTokens.value]]
  for (let i = 0; i < txrows.value.length; i++) {
    const row = [
      ...
      ...headerTokens.value.map(token => txrows.value[i][token.toUpperCase()] ?? "")
    ]
    rows.push(row)
  }
   ...
   <main>
     <div class="cont">
      <div class="title">
        ...
        <div class="buttons">
          ...
          <button id="titlebuttons" @click="pageBottom()">Go to bottom of the page</button>
        </div>
      </div>
      <div class="minicont">
     ...
     <div class="cont" v-if="loading == false" id="fadeIn">
      ...
      <div>
        <div class="buttons">
          <button id="titlebuttons" @click="downloadCSV()">Export CSV</button>
          <button id="titlebuttons" @click="pageTop()">Scroll to top of the page</button>
        </div>
      </div>
     </div>
