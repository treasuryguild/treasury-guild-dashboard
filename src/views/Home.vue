...
async function getMenu() {
  ...
  for (let i in store.allprojects) {
    ...
    if (store.allprojects[i].logo_url) {
      ...
      logo_url.value.push(
        "https://vgfuwxqfnkkqwyyospxi.supabase.co/storage/v1/object/sign/orgprofiles/teamwork.png?token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1cmwiOiJvcmdwcm9maWxlcy90ZWFtd29yay5wbmciLCJpYXQiOjE2NzYyNjQxMTcsImV4cCI6MTcwNzgwMDExN30.aJiWDJb9ikkW9wqjGk3Y2JGsJw__5h87DSRy9q2DEik&t=2023-02-13T04%3A55%3A17.494Z"
      );
    }
  }
...
<template>
  <main>
    <div class="cardcontainer">
      <RouterLink
        :to="`/${group}`"
        class="groups"
        v-for="group in sortedgroupname"
        :key="group"
      >
        <div class="images">
          <img
            :src="logo_url[groupname.indexOf(group)]"
  ...
  <footer>
    <a
      class="tribute"
      v-if="loading == false"
      href="https://www.flaticon.com/free-icons/organization"
      title="organization icons"
      >Organization icons created by xnimrodx - Flaticon</a
    >
  </footer>
</template>
