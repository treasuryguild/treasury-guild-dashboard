...
export async function useGetAllContributions(project_id) {
  ...
  async function getContributions() {  // still busy building and testing
    ...
    try {
      ...
      let { data, error, status } = await supabase
        .from('contributions')
        .select(`contribution_id, tx_id, task_creator, task_name, task_label, task_description, task_type`)
  ...
  return { all_contribution_id, all_transaction_id, all_task_creator, all_task_name, all_task_label, all_task_description, all_task_type }
}
