...
export async function useGetContributions(transaction_id) {
  ...
  async function getContributions() {  // still busy building and testing
    ...
    try {
      ...
      let { data, error, status } = await supabase
        .from('contributions')
        .select(`contribution_id, task_creator, task_name, task_label, task_description, task_type`)
  ...
  return { contribution_id, task_creator, task_name, task_label, task_description, task_type }
}
