  ...
  async function getWallet() {
    ...
    for (let i in data.tokens) {
      if (data.tokens[i].quantity == 1) {
        ...
        nftPicture.value.push(
          data.tokens[i].metadata.image instanceof Array
            ? "https://ipfs.io/ipfs/" +
                data.tokens[i].metadata.image.join("").slice(7)
            : "https://ipfs.io/ipfs/" + data.tokens[i].metadata.image.slice(7)
        );
      } else {
        tokens.value.push(data.tokens[i].metadata.ticker);
        tokenAmounts.value.push(
          (
            data.tokens[i].quantity /
            Math.pow(10, data.tokens[i].metadata.decimals)
          ).toFixed(6)
        );
      }
    }
