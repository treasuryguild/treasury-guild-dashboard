...
<template>
    <nav>
      ...
      <menuitem v-if="route.path.startsWith(`/${store.group}/${store.project}`)"><RouterLink :to="`/${store.group}`">Back to {{ store.group }}</RouterLink></menuitem>
      <menuitem><RouterLink to="/transaction" @click="onChange3()">Transaction</RouterLink></menuitem>
      <menuitem><RouterLink to="/wallet" @click="onChange3()">Wallet</RouterLink></menuitem>
		</menu>
	</nav>
</template>
